    ...
    private static void runProgram() {
        ...
        while (runProgram) {
            try {
                ...
                switch (input) {
                    case "1" -> testInputsForDatabase();
                    case "2" -> startExam(scanner);
...
//                    case "5" -> statistics(scanner);
    ...
    private static void printMenu() {
        System.out.println("""
                ...
                [2] - begin exam;
                [3] - add aditional question to the exam;
    ...
    private static void testInputsForDatabase() {
    ...
    private static void startExam(Scanner scanner) {
        System.out.println("There is only one correct answer. Begin exam");
        try (Session session = SessionFactoryMaker.getFactory().openSession()) {
            int counter = 1;
            while (session.get(Exams.class, counter) != null)
            System.out.println(session.get(Exams.class, counter++));
        }
    }
}